...
let project = parseArgv(process.argv, '--project') || 'demo';

console.log('project', project);

let testProjectFile = testFile(project);

const zip = parseArgv(process.argv, '--zip');

// console.log('zip', zip);

console.log('testFile', testFile(project, 'config.js'));
// console.log('testFile', JSON.parse(testProjectFile('config.js')).version);

if (!testFile(project, 'main.js')) {
    console.log(chalk.red.bgCyan.bold('--------项目缺少入口文件---------\n'));
    process.exit();
}

if (!testFile(project, 'config.js')) {
        ...
        if (zip === 'true' && isBuild) {
            config.plugin('filemanager').use(FileManagerPlugin, [{
                onEnd: {
                    archive: [
                        {
                            source: `${__dirname}/dist/${project}-${version}`,
                            destination: `${__dirname}/dist/${project}-${version}.zip`
                        },
                    ],
                },
            }])
